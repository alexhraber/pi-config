...
command -v python3 >/dev/null || { echo "pi-config requires python3 to update settings.json" >&2; exit 1; }
mkdir -p "${PI_DIR}/themes" "${PI_DIR}/prompts" "${PI_DIR}/extensions"

if [[ -f "${PI_DIR}/settings.json" ]]; then
  cp -p "${PI_DIR}/settings.json" "${PI_DIR}/settings.json.pi-config-backup.$(date +%Y%m%d%H%M%S)"
fi
...
try:
    ...
    raise SystemExit(f"Refusing to overwrite invalid JSON in {settings_path}: {exc}")
if not isinstance(data, dict):
    raise SystemExit(f"Refusing to overwrite non-object settings in {settings_path}")
